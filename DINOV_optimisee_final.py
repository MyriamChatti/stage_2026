...
def extract_attention(model, tensor, device):
    ...
    with torch.no_grad():
        feats = model.forward_features(tensor.to(device))
        cls = feats["x_norm_clstoken"]        # token global
        patches = feats["x_norm_patchtokens"] # tokens locaux

    attn = (patches @ cls.unsqueeze(-1)).squeeze(-1)
...
def segment(feat_map, img, attn):
    ...
    # PCA 
    X = PCA(min(64, X.shape[1])).fit_transform(X)
    ...
    # centre anatomique
    center_prior = np.exp(-((pos[:,0]-0.5)**2 + (pos[:,1]-0.5)**2)*5)
    ...
    labels = KMeans(n_clusters=N_REGIONS, n_init=20, random_state=42).fit_predict(X)

    seg = labels.reshape(h,w)
    seg = zoom(seg,(H/h,W/w),order=0)

    #REFINEMENT SUPERPIXEL
    superpix = slic(img, n_segments=400, compactness=10, start_label=0, channel_axis=None)

    seg_refined = np.zeros_like(seg)

    for s in np.unique(superpix):
        mask = superpix == s
        vals = seg[mask]
        seg_refined[mask] = np.bincount(vals).argmax()

    return seg_refined

# ============================================================
# ASSIGN ANATOMY (ROBUSTE)
# ============================================================
def assign(seg,img):
    ...
    def pick(f):
        valid = [p for p in rem if p['k'] not in used]
        b=min(valid,key=f)
        used.add(b['k'])
        rem.remove(b)
        return b['k']
