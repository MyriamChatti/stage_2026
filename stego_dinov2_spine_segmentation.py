"""
============================================================
  stego_dinov2_spine_segmentation.py

  Pipeline 100% non supervisé combinant STEGO + DINOv2
  pour segmenter les 10 régions spinales et paraspinales
  sur IRM T2 axiale lombaire + analyse texture Mannil.

  Combinaison STEGO + DINOv2 :
    - DINOv2 (ViT-S/14, 2023) remplace DINO v1 (ViT-S/8)
      → features bien plus riches (entraîné sur 142M images)
      → patch 14×14 = contexte anatomique plus large
    - STEGO entraîne une tête de segmentation légère
      sur les features DINOv2 via perte contrastive
      → adapte les features à vos IRM spécifiques

  Pourquoi DINOv2 est meilleur que DINO v1 pour l'IRM :
    - Patch 14×14 vs 8×8 : capture mieux les structures
      musculaires de taille intermédiaire
    - Features 384 dims entraînées sur 142M images vs 1.2M
    - Meilleure séparabilité des tissus mous

  Différence avec stego_spine_segmentation.py :
    - DINO v1 vits8  → DINOv2 vits14
    - patch 8×8      → patch 14×14
    - hub dino:main  → hub dinov2
    - reshape=True   → reshape=False (API DINOv2)
  ...
  Usage :
      python stego_dinov2_spine_segmentation.py
============================================================
"""
...
#Agg = pun backend qui sauvegrade directement en PNG sans ouvrir de fenêtre
...
OUTPUT_FOLDER = "/home/myriam/Documents/stage_M2/Stage_2026/code/MASKCONTRAST/stego_dinov2_results"
...
def visualize(img_orig, seg_stego, anat_map,
    ...
    fig = plt.figure(figsize=(28, 16),
                     facecolor='black')
    fig.suptitle(
        f"STEGO + DINOv2 + Texture Mannil 2018 "
        f"— {img_name}",
...
if __name__ == '__main__':
    ...
    print(f"[INFO] Modèle : DINOv2 vits14 "
          f"(patch 14, 384 dims)")
    ...
    for img_path in image_paths:
        ...
        fig_path = os.path.join(
            OUTPUT_FOLDER,
            f"{name}_stego_dinov2_mannil.png")
    ...
    if all_rows:
        ...
        csv_path = os.path.join(
            OUTPUT_FOLDER,
            'all_patients_stego_dinov2_mannil.csv')
