...
OUTPUT_FOLDER = "/home/myriam/Documents/stage_M2/Stage_2026/code/MASKCONTRAST/unsam_results_final"
...
def segment(img):
    ...
    segments = slic(
        ...
        compactness=5,
        start_label=0,
        channel_axis=None   # obligatoire pour IRM
    )
    ...
    km = KMeans(n_clusters=N_REGIONS, random_state=42, n_init=15)
...
#ici le plus important !
    # STRUCTURES
    sac_center = pop_best(lambda p: -p['mean']*8 + p['dist']*12)
    sac_periph = pop_best(lambda p: -p['mean']*5 + p['dist']*10)

    disc = pop_best(lambda p: abs(p['mean']-0.45)*6 + (p['y']-0.65)**2*8)

    eminence = pop_best(lambda p: p['mean']*2 + p['y']*6)
...
if __name__=="__main__":
    ...
    print(" UNSAM segmentation lancée...\n")
