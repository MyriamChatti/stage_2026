...
def call_model_function(images, call_model_args=None, expected_keys=None):
    ...
    # récupérer le score correspondant pour chaque image
    output = tf.gather(preds, class_idx, axis=1, batch_dims=1)

    return {
        saliency.OUTPUT_LAYER_VALUES: output.numpy()
    }

# méthode occlusion
occlusion = saliency.Occlusion()
...
for file in os.listdir(input_folder):
    ...
    if file.endswith(".png"):
        ...
        save_path = os.path.join(
            output_folder,
            file.replace(".png","_occlusion.png")
        )
