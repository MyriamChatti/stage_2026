...
# mapping modules (une seule fois suffit)
sys.modules['core'] = picie

# mapping classe exacte
picie.UnsupervisedMetrics = utils.UnsupervisedMetrics
...
def my_app(cfg: DictConfig) -> None:
    ...
    for model_path in cfg.model_paths:
        ...
        if run_picie:
            ...
            picie_state = torch.load("../saved_models/picie_and_probes.pth", map_location=torch.device("cpu"))
            #picie = picie_state["model"].cuda()
            #picie = picie_state["model"].to("cpu")
